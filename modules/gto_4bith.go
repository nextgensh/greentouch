...
func (m *Module_GTO4H) GetState() (int, bool) {
...
func (m *Module_GTO4H) HandleEvent(event lib.Event,
								data float64,
								spiketime float64,
								islte bool) bool {
	m.count ++;
	...
	if element, present := eventtable2h[event.Name]; present {
		...
		if islte {
			element.history = lib.BitAddN(element.history, lib.CLTE, 15);
			if lib.ShouldISwitch(data, spiketime) {
			...
			element.history = lib.BitAddN(element.history, lib.C3G, 15);
			element.predict_table[element.history] = lib.BitDec(
							element.predict_table[element.history]);
		}
...
func (m *Module_GTO4H) SetState(state int) bool {
	m.state = state;

	return true;
}

func (m *Module_GTO4H) ServeData(data float64) bool {
	if m.state == lib.C3G {
 ...
 */
func (m *Module_GTO4H) jumpAssist(prev_state int, current_state int,
...
func (m *Module_GTO4H) GetSwitchingEnergy() float64 {
	return m.switching_energy;
}

func (m *Module_GTO4H) GetDelayTransmission() float64 {
	return m.delay_transmission;
}

func (m *Module_GTO4H) GetDelayTransition() float64 {
	return m.delay_transition;
}

func (m *Module_GTO4H) GetAvgDelayTransition() float64 {
...
func (m *Module_GTO4H) GetAvgDelayTransmission() float64 {
	return 0.0;
}

func (m *Module_GTO4H) GetCorrect() int {
	return m.ltecount - (m.GetMissed() + m.GetUnnecesary());
}

func (m *Module_GTO4H) GetMissed() int {
	return m.missed;
}

func (m *Module_GTO4H) GetUnnecesary() int {
	return m.unnecesary;
}

func (m *Module_GTO4H) GetServe3G() int {
	return m.serve3g;
}

func (m *Module_GTO4H) GetTotal() int {
	return m.count;
}

func (m *Module_GTO4H) GetTotalLTE() int {
	return m.ltecount;
}

func (m *Module_GTO4H) Reset() bool {
	m.switch_flag = true;
	m.isfirstlteevent = true;
...
func (m *Module_GTO4H) GetFirstAvgDelayTransition() float64 {
	return m.first_delay_transition / float64(m.first_avg_count);
}

func (m *Module_GTO4H) GetHiddenLatency(spiketime float64) float64 {
	temp := lib.Switch_3gtolte - spiketime;
	if temp < 0.0 {
		temp = 0.0;
	}
