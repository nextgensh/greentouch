...
func (m *Module_GTO5) GetState() (int, bool) {
...
func (m *Module_GTO5) HandleEvent(event lib.Event,
								data float64,
								spiketime float64,
								islte bool) bool {
	...
	if found {
		if lib.BitJumpN(element.counter, 16) {
		...
		eventtable2[event.Name] = EventSwitch2{31};
	}
	...
	if element, present := eventtable2[event.Name]; present {
		...
		if islte {
			if lib.ShouldISwitch(data, spiketime) {
				element.counter = lib.BitIncN(element.counter, 31);
			} else {
...
func (m *Module_GTO5) SetState(state int) bool {
	m.state = state;

	return true;
}

func (m *Module_GTO5) ServeData(data float64) bool {
	if m.state == lib.C3G {
 ...
 */
func (m *Module_GTO5) jumpAssist(prev_state int, current_state int,
...
func (m *Module_GTO5) GetSwitchingEnergy() float64 {
	return m.switching_energy;
}

func (m *Module_GTO5) GetDelayTransmission() float64 {
	return m.delay_transmission;
}

func (m *Module_GTO5) GetDelayTransition() float64 {
	return m.delay_transition;
}

func (m *Module_GTO5) GetAvgDelayTransition() float64 {
	return m.GetDelayTransition() / float64(m.avg_count);
}

func (m *Module_GTO5) GetAvgDelayTransmission() float64 {
	return 0.0;
}

func (m *Module_GTO5) GetCorrect() int {
	return m.ltecount - (m.GetMissed() + m.GetUnnecesary());
}

func (m *Module_GTO5) GetMissed() int {
	return m.missed;
}

func (m *Module_GTO5) GetUnnecesary() int {
	return m.unnecesary;
}

func (m *Module_GTO5) GetServe3G() int {
	return m.serve3g;
}

func (m *Module_GTO5) GetTotal() int {
	return m.count;
}

func (m *Module_GTO5) GetTotalLTE() int {
	return m.ltecount;
}

func (m *Module_GTO5) Reset() bool {
...
func (m *Module_GTO5) GetFirstAvgDelayTransition() float64 {
	panic("unimplemented method");
